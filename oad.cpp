...
std::string oad::setSpeed(double tcp, double ori, double leax, double reax)
{
  ...
  msg += buff ;
  sprintf_s(buff,"%08.1lf ",leax);
  msg += buff ;
  sprintf_s(buff,"%08.2lf ",reax);
  msg += buff ;
  msg += "#";
...
{
  int ok;
  sscanf_s(msg.c_str(),"%*d %d %lf %lf %lf %lf %lf %lf %lf",&ok,x,y,z,q0,qx,qy,qz);
  return ok;
}
...
{
  int ok;
  sscanf_s(msg.c_str(),"%*d %d %lf %lf %lf %lf %lf %lf",&ok,joint1,joint2,joint3,joint4,joint5,joint6);
  return ok;
}
